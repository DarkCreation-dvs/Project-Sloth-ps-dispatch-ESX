...
local inHuntingZone, inNoDispatchZone = false, false
...
print("[ps-dispatch] ESX initialized!")

-- Setup dispatch UI and data
local function setupDispatch()
    local playerInfo = ESX.GetPlayerData()
    if not playerInfo then
        print("[ps-dispatch] Failed to get player data!")
        return
    end
    ...
    PlayerData = {
        charinfo = {
            firstname = playerInfo.charinfo.firstname or "Unknown",
            lastname = playerInfo.charinfo.lastname or "Unknown"
        },
        metadata = {
            callsign = playerInfo.metadata and playerInfo.metadata.callsign or "N/A"
        },
        citizenid = playerInfo.identifier or "N/A",
        job = {
            type = playerInfo.job and playerInfo.job.grade_name or "unknown",
            name = playerInfo.job and playerInfo.job.name or "unknown",
            label = playerInfo.job and playerInfo.job.label or "Unknown"
        },
    }
...
-- Example event handlers for ESX
...
AddEventHandler('esx:playerLoaded', function()
    setupDispatch()
    -- createZones() -- add your zone creation if any
end)
...
-- Functions
---@param bool boolean Toggles visibility of the menu
local function toggleUI(bool)
    SetNuiFocus(bool, bool)
    SendNUIMessage({ action = "setVisible", data = bool })
end
...
local function setupDispatch()
    ESX.TriggerServerCallback('ps-dispatch:getPlayerData', function(playerInfo)
        if not playerInfo then
            print("[ps-dispatch] Failed to get player data!")
            return
        end
        ...
        PlayerData = {
            ...
            metadata = {
                callsign = playerInfo.metadata.callsign or "N/A"
            },
            citizenid = playerInfo.citizenid or "N/A",
            job = {
                type = playerInfo.job.type or "unknown",
                name = playerInfo.job.name or "unknown",
                label = playerInfo.job.label or "Unknown"
            },
        }
...
RegisterNetEvent('esx:setJob', function(job)
    -- Update PlayerData.job if needed, then call setupDispatch to refresh UI
    PlayerData.job = job
    setupDispatch()
end)
